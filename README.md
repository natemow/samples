...
1. TOP PRIORITY Confirm that Eloqua integration works, possible need to re-config stuff.
  ...
  1. `status: 10/3 IN PROGRESS. TOKENIZED SUBJECT LINE NOW EDITABLE FROM GUI AND ALTERING DOES CORRECTLY GET PUSHED TO ELOQUA. STILL NEED TO WORK OUT PERSISTENT STORAGE OF SUBJECT CHANGES.` Add Eloqua custom subject line to emails.
    <pre>
...
1. `status: IN PROGRESS FOR WEEK OF 9/22` Martyn to revamp email template, Socha to integrate to v2.1.3 @see pm/views/templates/alerts, pm/views/templates/header and determine how/where Martyn needs to implement, determine what customization of php vars is available to him i.e. $body $header.
  </pre>
